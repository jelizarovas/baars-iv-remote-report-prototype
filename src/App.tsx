...
const routeHash: Record<View,string> = {welcome:'',setup:'#start',share:'#share',questions:'#questions',review:'#review'};
const viewFromLocation=():View=>invitationFromHash()?'setup':location.hash==='#share'?'share':location.hash==='#start'?'setup':location.hash==='#questions'?'questions':location.hash==='#review'?'review':'welcome';
...
export default function App(){
 ...
 const [session,setSession]=useState<Session|null>(null);const [view,setView]=useState<View>(viewFromLocation);const [drawer,setDrawer]=useState(false);const [invalid,setInvalid]=useState(false);const [message,setMessage]=useState('');const [saveState,setSaveState]=useState<'saved'|'saving'>('saved');const saveTimer=useRef<number|undefined>(undefined);
 const navigate=(next:View,url=routeHash[next],replace=false)=>{const state:RouteState={baarsRoute:next,from:view};history[replace?'replaceState':'pushState'](state,'',url||location.pathname);setView(next);setDrawer(false);setInvalid(false);window.scrollTo(0,0)};
 const backTo=(fallback:View)=>{const state=history.state as RouteState|null;if(state?.from)history.back();else navigate(fallback,routeHash[fallback],true)};
 useEffect(()=>{const syncRoute=()=>{setView(viewFromLocation());setDrawer(false);setInvalid(false);window.scrollTo(0,0)};window.addEventListener('popstate',syncRoute);window.addEventListener('hashchange',syncRoute);return()=>{window.removeEventListener('popstate',syncRoute);window.removeEventListener('hashchange',syncRoute)}},[]);
