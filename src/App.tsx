...
export default function App(){
 ...
 const jump=(id:string)=>{if(!session)return;const update=()=>{setSession({...session,currentQuestionId:id});setView('questions');setDrawer(false);setInvalid(false);window.scrollTo(0,0)};if(view==='questions')update();else runTransition(update)};
 ...
 if(view==='share')return <Share onClose={()=>navigate('welcome',location.pathname)}/>;
 if(!session&&view==='welcome')return <Welcome onStart={()=>navigate('setup','#start')} onInvite={()=>navigate('share','#share')}/>;
 if(!session)return <Setup initial={invitation?respondentFromInvitation(invitation):undefined} onStart={value=>runTransition(()=>{setLanguage(value.respondent.language??language);setSession(value);setView('questions')})} onBack={()=>navigate('welcome',location.pathname)} onOpenShare={()=>navigate('share','#share')}/>;
 if(view==='review')return <Review session={session} onJump={jump} onBack={()=>navigate('questions')}/>;
 ...
 {drawer&&<Drawer session={session} current={q.id} onJump={jump} onClose={()=>setDrawer(false)} onReview={()=>runTransition(()=>{setDrawer(false);setView('review')})} onExport={exportJson} onImport={importJson} onClear={()=>{if(confirm(language==='en'?'Are you sure you want to clear every answer in this session?':'Ar tikrai norite išvalyti visus šios sesijos atsakymus?'))runTransition(()=>{clearSession(session);setSession(null);setView('welcome')})}}/>}</div>;
}
