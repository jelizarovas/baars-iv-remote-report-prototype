...
import { localized, questionText, showEnglish, t } from './i18n';
...
export default function App(){
 ...
 const language=session?.respondent.language??'lt-en';
 ...
 const importJson=async(file:File)=>{try{const data=JSON.parse(await file.text()) as Session;if(data.version!==1||!data.respondent||!data.answers)throw new Error();setSession(data);setDrawer(false)}catch{alert('Nepavyko importuoti: netinkamas BAARS-IV JSON failas.')}};
 ...
 if(!session)return <Setup initial={invitation?respondentFromInvitation(invitation):undefined} onStart={value=>{setSession(value);setView('questions')}} onBack={()=>{history.replaceState(null,'',location.pathname);setView('welcome')}} onOpenShare={()=>{history.replaceState(null,'','#share');setView('share')}}/>;
 ...
 <main className="question-area"><article className="question-card"><div className="eyebrow"><span>{section.primary}{section.secondary&&<small>{section.secondary}</small>}</span><b>{t(language,'question')} {q.number}<small>{position} / 61</small></b></div>{timeframe&&<div className="timeframe">◷ <span>{timeframe.primary}{timeframe.secondary&&<small>{timeframe.secondary}</small>}</span></div>}<h1>{questionText(q,language)}</h1>{showEnglish(language)&&<p className="english">{q.en}</p>}{suggestion!==undefined&&<button className="suggestion" onClick={()=>setAnswer(suggestion)}>{t(language,'suggestion')}: <strong>{suggestion?t(language,'yes'):t(language,'no')}</strong><span>{t(language,'confirmSuggestion')}</span></button>}<QuestionControl question={q} answer={answer} onChange={setAnswer} invalid={invalid} language={language}/>{message&&<p className="blocked" role="alert">{message}</p>}</article></main>
 <nav className="bottom-nav" aria-label="Klausimyno navigacija"><button onClick={()=>go(-1)} disabled={position===1}>← <span>{t(language,'back')}</span></button>{q.type==='frequency'&&<div className="nav-scores">{[1,2,3,4].map(n=><button key={n} className={answer===n?'selected':''} aria-label={`Pasirinkti ${n}`} onClick={()=>setAnswer(n)}>{n}</button>)}</div>}<button className="next" onClick={()=>go(1)}><span>{t(language,'next')}</span> →</button></nav>
 {drawer&&<Drawer session={session} current={q.id} onJump={jump} onClose={()=>setDrawer(false)} onReview={()=>{setDrawer(false);setView('review')}} onExport={exportJson} onImport={importJson} onClear={()=>{if(confirm('Ar tikrai norite išvalyti visus šios sesijos atsakymus?')){clearSession(session);setSession(null);setView('welcome')}}}/>}</div>;
}
