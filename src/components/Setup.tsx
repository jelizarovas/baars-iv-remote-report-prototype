...
export function Setup({onStart,initial,onOpenShare,onBack}:{onStart:(s:Session)=>void;initial?:Respondent;onOpenShare:()=>void;onBack:()=>void}) {
 ...
 const copy = language === 'en' ? {
  ...
  asked:(name:string,person:string)=>`${name}, you have been asked to rate ${person}.`, person:'Name of person being rated', personPlaceholder:'e.g. Alex Morgan', respondent:'Your full name', respondentPlaceholder:'e.g. Jordan Lee', relationship:'Relationship to the person',
  relationshipPlaceholder:'Choose or type', date:'Date', returnTo:'Return completed form to',
  ...
  asked:(name:string,person:string)=>`${name}, prašoma įvertinti: ${person}.`, person:'Vertinamo asmens vardas', personPlaceholder:'pvz., Jonas Jonaitis', respondent:'Jūsų vardas ir pavardė', respondentPlaceholder:'pvz., Ona Onaitė', relationship:'Ryšys su vertinamu asmeniu',
  relationshipPlaceholder:'Pasirinkite arba įrašykite', date:'Data', returnTo:'Užpildytą formą grąžinti',
 ...
 const existing = currentForm.personName.trim() && currentForm.respondentName.trim() ? loadSession(currentForm) : undefined;
 ...
 return <main className="setup-shell material-page"><div className="mobile-route-toolbar"><button className="text-button" onClick={onBack}>← {copy.about}</button><div className="brand-mark">BAARS-IV</div></div><section className="setup-card">
  ...
  <div className="form-grid">
   <label>{copy.person}<input placeholder={copy.personPlaceholder} value={currentForm.personName} onChange={e=>update('personName',e.target.value)} autoComplete="off" /></label>
   <label>{copy.respondent}<input placeholder={copy.respondentPlaceholder} value={currentForm.respondentName} onChange={e=>update('respondentName',e.target.value)} autoComplete="name" /></label>
  ...
  <div className="setup-actions"><button className="primary" onClick={()=>start(true)}>{copy.start}</button>{existing&&<><button onClick={()=>start(false)}>{copy.resume}</button><button onClick={()=>{if(valid()&&confirm(copy.restartConfirm))start(true)}}>{copy.restart}</button></>}</div>
