...
export function Review({session,onJump,onBack}:{session:Session;onJump:(id:string)=>void;onBack:()=>void}){
 ...
 return <main className="review"><header><button onClick={onBack}>← {en?'Back':'Grįžti'}</button><div><strong>{en?'Review':'Peržiūra'}</strong></div><div className="review-percent">{progress.percent}%</div></header><div className="review-body">
